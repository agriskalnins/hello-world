	...
	echo "
			...
			/*Atrod raksta galeriju*/
					$idn=$row['id_not']; //iegūst notikuma id
		   			$result_gal = mysql_query("SELECT gal_id_not, gal_id_gal FROM web_notikumi_gal WHERE gal_id_not=$idn");	//atlasa notikuma galerija kur ir galerijas id pēc notikuma id
					...
					else 
					{
						...
						$result_galeri = mysql_query("SELECT * FROM web_gal WHERE id_gal=$idg"); //atlasa galeriju ar tādu pašu id kads bija pie notikuma	
						$row_galeri = mysql_fetch_array($result_galeri);
						...
						$result_bildes = mysql_query("SELECT * FROM web_gal_img WHERE gal_id=$idg"); //atlasa bildes kas bija pie šīs galerijas id
						
						echo "<ul class='gallery clearfix'>";
						while ($row_bildes= mysql_fetch_array($result_bildes))
						{
			...
			/*Atrod raksta pielikumus*/
					...
					$count = @mysql_num_rows($result_fil);
