...
mysql_query('SET NAMES cp1251') or die (mysql_error($connection));
...
$query_select_galerija = "SELECT gal_nos_ru, gal_date, COUNT(img) AS count FROM web_gal, web_gal_img WHERE gal_id=id_gal AND gal_id=".$_GET['gid']." GROUP BY id_gal";
...
<TR>
	...
	<TD width="95%" style="font-size: 12pt; font-family: Arial; font-weight: Bold" height="28">'.$gal_nos_ru.' '.$gal_date.'
	</TD>
</TR>
</table>';
//---------------------------------
...
echo "<p style='margin-top:0; margin-bottom:9px; margin-left:24px'><a class=href3 href='javascript:history.back(1)'>&nbsp;<img border=0 src=../ukadm/images/backbullet.png>&nbsp;Назад</a></p>";
	...
	$gal_date $gal_nos_ru. $nr из $count
</div>
...
$query_select_galerija = "SELECT id_gal, gal_nos_ru, gal_date, COUNT(img) AS count FROM web_gal, web_gal_img WHERE gal_id=id_gal AND LENGTH(gal_nos_ru)>0 GROUP BY id_gal ORDER BY gal_date DESC";
$result_select_galerija = mysql_query($query_select_galerija, $connection) or die (mysql_error($connection));
...
echo "
	...
	<table class=list cellspacing=0 cellpadding=4>";	
		...
		{
		echo "<tr height=26>
			<td align=center colspan=3 class=tcont1>- saraksts ir tuks -</td>
			</tr>";
		}
	...
	<td class=list align=left valign=middle width=500><span style='font-size: 11px; font-family:Tahoma; color: #5A5A5A'><div>$gal_date</div></span><a class=href3 href=$url&gid=$id_gal>$gal_nos_ru</a></td>
